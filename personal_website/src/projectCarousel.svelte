...
<script lang="ts">
    ...
    async function nextProject() {
      ...
      for (let i = 0; i <= 100; i += 4) {
        ...
        opacities[1] = Math.max(0.6, startOpacities[1] * (1 - progress) + 0.6 * progress); // current dims
    ...
    async function prevProject() {
      ...
      for (let i = 0; i <= 100; i += 4) {
        ...
        opacities[1] = Math.max(0.6, startOpacities[0] * (1 - progress) + 1 * progress); // prev becomes fully visible
        opacities[2] = Math.max(0.6, startOpacities[1] * (1 - progress) + 0.6 * progress); // current dims
  ...
  <div class="carousel-container w-full mt-auto mb-8"> 
    
    <div class="carousel-content">
      <div class="flex items-center justify-between mb-4">
        <button 
          class="carousel-nav-btn bg-green-700 hover:bg-green-600 text-white p-3 rounded-full shadow-lg transition-colors duration-200 z-10"
          onclick={prevProject}
        ...
        <div class="carousel-items flex-1 mx-4 overflow-hidden relative h-[160px]">
            <div class="carousel-track absolute inset-0 flex justify-center items-center">
                ...
                <div class="carousel-item absolute"
                     style="transform: translateX({positions[positions.length > 3 ? 1 : 0]}%) scale({scales[positions.length > 3 ? 1 : 0]}); opacity: {opacities[positions.length > 3 ? 1 : 0]};">
                  <button 
                    class="hexagon-btn relative
                      {visualFocusIndex === getProjectIndex(-1) 
                        ? 'bg-green-500' 
                        : 'bg-green-700 hover:bg-green-600'}"
                    onmouseenter={() => !isTransitioning && onProjectHover(projects[getProjectIndex(-1)])}
                    onmouseleave={() => !isTransitioning && onProjectHover(getCurrentProject())}
                ...
                <div class="carousel-item absolute z-10"
                     style="transform: translateX({positions[positions.length > 3 ? 2 : 1]}%) scale({scales[positions.length > 3 ? 2 : 1]}); opacity: {opacities[positions.length > 3 ? 2 : 1]};">
                  <button 
                    class="hexagon-btn relative
                      {visualFocusIndex === currentIndex
                        ? 'bg-green-500'
                        : 'bg-green-700 hover:bg-green-600'}"
                    onmouseenter={() => !isTransitioning && onProjectHover(getCurrentProject())}
                    onmouseleave={() => !isTransitioning && onProjectHover(getCurrentProject())}
                    onclick={() => !isTransitioning && onProjectSelect(projects[currentIndex])}
                  >
                ...
                <!-- Next Project (Right) -->
                <div class="carousel-item absolute"
                     style="transform: translateX({positions[positions.length > 3 ? 3 : 2]}%) scale({scales[positions.length > 3 ? 3 : 2]}); opacity: {opacities[positions.length > 3 ? 3 : 2]};">
                  <button 
                    class="hexagon-btn relative
                      {visualFocusIndex === getProjectIndex(1)
                        ? 'bg-green-500'
                ...
                {#if positions.length > 3}
                  <div class="carousel-item absolute"
                       style="transform: translateX({positions[positions.length - 1]}%) scale({scales[scales.length - 1]}); opacity: {opacities[opacities.length - 1]};">
                    <button 
                      class="hexagon-btn relative
                        {visualFocusIndex === getProjectIndex(2) 
                          ? 'bg-green-500' 
      ...
      <div class="carousel-indicators flex justify-center space-x-2 mt-4">
        {#each projects as _, i}
          <button 
            class="h-2 w-8 rounded-full transition-colors duration-200 {i === currentIndex ? 'bg-green-500' : 'bg-gray-600 hover:bg-gray-500'}"
            onclick={async () => {
